...
legend('Trajectory', 'Start (t=0)', 'End (t=4s)', ...
       'Location', 'northwest', 'FontSize', 9);
grid on; box off;
sgtitle('Test 1: Pure Roll Torque from Rest (Open-Loop, No Controller)', ...
...
title({'\theta — pitch coupling  (I_{zz}-I_{xx}) \cdot \phi_{dot} \cdot \psi_{dot} / I_{yy}', ...
       'pitch responds even though \tau_\theta = 0 — this is the non-linearity'}, ...
       'FontSize', 10);
grid on; box off;
...
sgtitle({'Test 2: Gyroscopic Coupling Demonstration', ...
...
if check1 && check2 && check3
    ...
    fprintf('  Ovearll: One or More Checks Failed\n');
    fprintf('\n');
